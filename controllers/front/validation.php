 ...
 */
class SavvyValidationModuleFrontController extends ModuleFrontController
{
    /**
    ...
    public function postProcess()
    {
        ...
        if (!$cart->id_customer || !$cart->id_address_delivery || !$cart->id_address_invoice || !$this->module->active) {
            $logMessage = sprintf(
                'Savvy: order validation failed. id_customer: %s, id_address_delivery: %s, id_address_invoice: %s',
                $cart->id_customer,
        ...
        if (!Validate::isLoadedObject($customer)) {
            $logMessage = sprintf('Savvy: order validation failed. Customer not found (%s)', $cart->id_customer);
            PrestaShopLogger::addLog($logMessage, 1, null, 'Cart', $cart->id, true);
            Tools::redirect('index.php?controller=order&step=1');
        }
        ...
        $this->module->validateOrder($cart->id, Configuration::get('SAVVY_OS_WAITING'), 0, $this->module->displayName, NULL, $mailVars, (int)$currency->id, false, $customer->secure_key);
